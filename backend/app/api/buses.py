...
async def create_bus_gps(
    ...
    """
    Create a GPS log entry for a bus.
    If no timestamp is provided, the backend uses the current UTC time.
    """
    result = await db.execute(select(Bus).where(Bus.id == bus_id))
    bus = result.scalars().first()
    if not bus:
...
async def get_latest_bus_gps(
    ...
    """
    Get the most recent GPS report for a bus.
    """
    result = await db.execute(
        select(GpsLog)
        .where(GpsLog.bus_id == bus_id)
